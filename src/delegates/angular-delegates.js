			...
			dgEventDirectives[dirName] = ['$parse', function($parse) {
				...
				return function(scope, ele, attrs) {
					...
					function parseFn(events, eventName) {
						...
						var lastTimes;
						if (eventName == 'mouseover' || eventName == 'mouseout' || eventName == 'mousemove' || eventName == 'mouseenter' || eventName == 'mouseleave') {
							ele.on(eventName, function(e) {
								if (lastTimes) clearTimeout(lastTimes);
								lastTimes = setTimeout(function() {
									var target = e.target;
									var el;
									if (
										(el = getClosest(
